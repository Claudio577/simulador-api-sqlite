...
# Requisitos: Python 3.10+ (usa apenas bibliotecas padrão)
...
def rand_date(start="2023-01-01", end="2025-08-01"):
    ...
    # random.randrange de 0..(days) para garantir data discreta
...
def main():
    if not SCHEMA_FILE.exists():
        raise SystemExit("ERRO: schema.sql não encontrado na raiz do repositório.")

    # cria/abre o banco e aplica o schema
    ...
    for i, eid in enumerate(event_ids, 1):
        ...
        events_buf.append((eid, name, start.date().isoformat(), end.date().isoformat(), price, seats))
        cur.execute("INSERT INTO events(event_id,name,start_date,end_date,price,seats) VALUES (?,?,?,?,?,?)", events_buf[-1])
    ...
    for p in payments_buf:
        cur.execute("INSERT INTO payments(payment_id,invoice_id,date,method,amount,conciliated,gateway_txid) VALUES (?,?,?,?,?,?,?)", p)
    ...
    for r in regs_buf:
        cur.execute("INSERT INTO registrations(registration_id,event_id,associate_id,date,paid) VALUES (?,?,?,?,?)", r)
    ...
    con.commit()
    con.close()
    print(f"✔ Banco gerado: {DB_PATH.resolve()}")
